...
class RegionMask(object):
    """
    Mask out random regions of the point cloud by dividing space into grid patches.
    This simulates structural missing parts in the point cloud.

    Args:
        mask_size: Size of each grid patch (default: 0.4)
        mask_ratio: Ratio of patches to mask out (default: 0.3, means 30% of patches will be masked)
        mask_application_ratio: Probability of applying this mask (default: 1.0, always apply)
        seed: Random seed for reproducibility (default: 42)
    """
    def __init__(self, mask_size=0.4, mask_ratio=0.3, mask_application_ratio=1.0, seed=42):
        self.mask_size = mask_size
        self.mask_ratio = mask_ratio
        self.mask_application_ratio = mask_application_ratio
        self.seed = seed
        self.call_count = 0  # Counter to ensure different masks for different samples

    def __call__(self, data_dict):
        # Set seed based on call count to ensure reproducibility but different masks per sample
        if self.seed is not None:
            np.random.seed(self.seed + self.call_count)
            random.seed(self.seed + self.call_count)
            self.call_count += 1

        if random.random() < self.mask_application_ratio:
            assert "coord" in data_dict.keys()
            coord = data_dict["coord"]

            # Get minimum coordinate for each dimension
            min_coord = coord.min(axis=0)

            # Divide space into grid patches
            grid_coord = ((coord - min_coord) // self.mask_size).astype(np.int32)

            # Find unique patches
            unique_patches = np.unique(grid_coord, axis=0)
            patch_num = len(unique_patches)

            # Randomly select patches to mask
            mask_patch_num = int(patch_num * self.mask_ratio)
            if mask_patch_num > 0:
                # Random select patches to mask out
                mask_patch_indices = np.random.choice(
                    patch_num, mask_patch_num, replace=False
                )
                mask_patches = unique_patches[mask_patch_indices]

                # Create mask: True means keep, False means mask out
                point_mask = np.ones(len(coord), dtype=bool)
                for mask_patch in mask_patches:
                    # Find all points in this patch
                    in_patch = np.all(grid_coord == mask_patch, axis=1)
                    point_mask[in_patch] = False

                # Get indices to keep
                keep_indices = np.where(point_mask)[0]

                # Handle sampled_index for data efficient mode
                if "sampled_index" in data_dict:
                    keep_indices = np.unique(np.append(keep_indices, data_dict["sampled_index"]))
                    mask = np.zeros_like(data_dict["segment"]).astype(bool)
                    mask[data_dict["sampled_index"]] = True
                    data_dict["sampled_index"] = np.where(mask[keep_indices])[0]

                # Apply mask to all fields (simple and clean!)
                data_dict = index_operator(data_dict, keep_indices)

...
@MODELS.register_module()
class DefaultSegmentorV2_domain(nn.Module):
    """
    ...
    def forward(self, data_dict):
        ...
        # 初始化熵的损失
        # data_dict["entropy_loss"] = torch.tensor(0,dtype=torch.float32,device=data_dict["coord"].device)
        # data_dict["balance_loss"] = torch.tensor(0,dtype=torch.float32,device=data_dict["coord"].device)
        if self.freeze_backbone:
            with torch.no_grad():
